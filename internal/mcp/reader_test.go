...
func TestGeminiReader_StdioServer(t *testing.T) {
	input := `{
  "mcpServers": {
    "github": {
      "command": "npx",
...
func TestGeminiReader_HTTPServer(t *testing.T) {
	input := `{
  "mcpServers": {
    "api": {
      "url": "https://mcp.example.com/v1",
...
func TestGeminiReader_MalformedJSON(t *testing.T) {
	r := &GeminiReader{}
	_, _, err := r.Read([]byte("{bad"))
	if err == nil {
		t.Fatal("expected error for malformed JSON")
...
// TestRoundTrip_GeminiReadWrite reads Gemini config and round-trips it.
func TestRoundTrip_GeminiReadWrite(t *testing.T) {
	input := `{
  "mcpServers": {
    "github": {
      "command": "npx",
